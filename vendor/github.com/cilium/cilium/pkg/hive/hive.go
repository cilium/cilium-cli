...
func (h *Hive) waitForSignalOrShutdown() error {
	...
	// equivalent to unix.SIGINT and unix.SIGTERM, but works on cross compilation for Windows
	signal.Notify(signals, os.Interrupt, syscall.Signal(0x2), syscall.Signal(0xf))
	select {
	case sig := <-signals:
