...
func (s *bgpAdvertisements) deleteK8sResources(ctx context.Context, t *check.Test) {
	ciliumClient := t.Context().K8sClient().CiliumClientset.CiliumV2alpha1()

	if s.bgpAPIVersion == 1 {
		ciliumClient.CiliumBGPPeeringPolicies().Delete(ctx, bgpPeeringPolicyName, metav1.DeleteOptions{})
	} else {
		ciliumClient.CiliumBGPClusterConfigs().Delete(ctx, bgpClusterConfigName, metav1.DeleteOptions{})
		ciliumClient.CiliumBGPPeerConfigs().Delete(ctx, bgpPeerConfigName, metav1.DeleteOptions{})
		ciliumClient.CiliumBGPAdvertisements().Delete(ctx, bgpAdvertisementName, metav1.DeleteOptions{})
	}
}

func (s *bgpAdvertisements) configureBGPv1Peering(ctx context.Context, t *check.Test, ipFamily features.IPFamily) {
	ct := t.Context()
	ciliumClient := ct.K8sClient().CiliumClientset.CiliumV2alpha1()
	s.deleteK8sResources(ctx, t)
	...
	_, err := ciliumClient.CiliumBGPPeeringPolicies().Create(ctx, peeringPolicy, metav1.CreateOptions{})
	if err != nil {
...
func (s *bgpAdvertisements) configureBGPv2Peering(ctx context.Context, t *check.Test, ipFamily features.IPFamily) {
	...
	_, err := ciliumClient.CiliumBGPAdvertisements().Create(ctx, advertisement, metav1.CreateOptions{})
	if err != nil {
	...
	_, err = ciliumClient.CiliumBGPPeerConfigs().Create(ctx, peerConfig, metav1.CreateOptions{})
	if err != nil {
	...
	_, err = ciliumClient.CiliumBGPClusterConfigs().Create(ctx, clusterConfig, metav1.CreateOptions{})
	if err != nil {
