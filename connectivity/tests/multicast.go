...
func (s *socatMulticast) populateMaps(ctx context.Context, t *check.Test, ciliumPods []corev1.Pod, ipToPodMap map[v2.NodeAddress]string) error {
	var wg sync.WaitGroup
...
func (s *socatMulticast) addAllNodes(ctx context.Context, t *check.Test) error {
	...
	if err := s.populateMaps(ctx, t, ciliumPods, ipToPodMap); err != nil {
		return err
	}
